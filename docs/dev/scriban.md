...
* Все циклы поддерживают операторы **break** (прерывание цикла) и **continue** (переход с следующему шагу). Например:
```
{{ for i in 1..5
    if i == 2
        break
...
* Например, встроенный объект **date.now** содержит в себе информацию о текущей дате. Его можно использовать следующим образом:
```
{{ date.now.‹field› }},
```
